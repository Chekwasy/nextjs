...
export default function Two2Win() {
    ...
    const [two2win, setTwo2win] = useState<Two2winEntry[]>([{
        time: '',
        Sbal: '0',
        stake: '0',
        odd: '0',
     ...
     * @param year The year.
     */
    const fetchTwo2WinData = (day: string, month: string, year: string) => {
        if (!day) return;

        const formattedDate = `${day.padStart(2, '0')}${month}${year}`;
        axios.get(`/api/gettwo2win?date=${formattedDate}`, {
    ...
    useEffect(() => {
        ...
        .then((response) => {
            ...
            fetchTwo2WinData(day.toString(), monthName, year.toString());
        })
        .catch(error => {
        ...
        <div className="flex flex-col items-center mt-16 min-h-screen bg-gray-100 py-8 px-4">
            ...
            {!showGuide && (
                <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg p-6 mb-6">
                    ...
                    {two2win.length > 0 && two2win[0].time !== '' ? (
                        two2win.map((entry, idx) => (
                ...
                <div className="bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-40 overflow-y-auto">
                    <div className="bg-white rounded-xl shadow-2xl p-6 md:p-8 max-w-3xl w-full my-8 relative">
                        ...
                        <div className="text-lg md:text-xl text-gray-700 leading-relaxed space-y-6">
                            ...
                            <p>
                            <strong>Match Structure and Odds:</strong> This schedule comprises one match, with a minimum total odd of 2.
                            </p>
                            <p>
                                    ...
                                    <div key={index} className="contents">
                                        {week.map((day, idx) => (
                                            <button
                                                ...
                                                onClick={() => {
                                                    if (day !== '') {
                                                        fetchTwo2WinData(day.toString(), selectedMonth, selectedYear);
                                                    }
                                                }}
                                                disabled={day === ''}
                                            >
