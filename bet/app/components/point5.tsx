...
// Define the interface for a single Point5 entry
interface Point5Entry {
    time: string;
    Sbal: string;
    stake: string;
    odd: string;
...
export default function Point5() {
    ...
    const [point5, setPoint5] = useState<Point5Entry[]>([{
        time: '',
        Sbal: '0',
        stake: '0',
        odd: '0',
     ...
     * @param year The year.
     */
    const fetchPoint5Data = (day: string, month: string, year: string) => {
        if (!day) return;

        const formattedDate = `${day.padStart(2, '0')}${month}${year}`;
        axios.get(`/api/getpoint5?date=${formattedDate}`, {
            headers: { tok: Cookies.get('trybet_tok') },
        })
        .then((response) => {
            if (response.data.game) {
                setPoint5(response.data.game);
                setSelectedDay(day);
        ...
        .catch(error => {
            console.error('Error fetching Point5 data:', error.message);
    ...
    useEffect(() => {
        ...
        .then((response) => {
            ...
            fetchPoint5Data(day.toString(), monthName, year.toString());
        })
        .catch(error => {
        ...
        <div className="flex flex-col items-center mt-16 min-h-screen bg-gray-100 py-8 px-4">
            ...
            {!showGuide && (
                <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg p-6 mb-6">
                    ...
                    {point5.length > 0 && point5[0].time !== '' ? (
                        point5.map((entry, idx) => (
                            <div key={idx} className="mb-8 p-4 bg-gray-50 rounded-lg shadow-sm border border-gray-200">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                                    ...
                                    <div className="flex justify-between bg-lime-50 rounded-lg p-3 col-span-full">
                                        ...
                                        <span className="font-bold text-lg text-yellow-700">{(((parseFloat(entry.Sbal) / 18000) * 100) - 100).toFixed(2)} %</span>
                                    </div>
                                    <div className="flex justify-between bg-lime-50 rounded-lg p-3 col-span-full">
                ...
                <div className="bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-40 overflow-y-auto">
                    <div className="bg-white rounded-xl shadow-2xl p-6 md:p-8 max-w-3xl w-full my-8 relative">
                        ...
                        <div className="text-lg md:text-xl text-gray-700 leading-relaxed space-y-6">
                            <p>
                            <strong>Introduction to This Betting System:</strong> This betting system operates with a principal (starting capital) of ₦18,000 and a minimum stake of ₦100. This foundational amount can be adjusted proportionally to achieve varying returns. For instance, a ₦1,800 principal would use a minimum stake of ₦10, and a ₦18,000 principal would use a ₦100 minimum stake.
                            </p>
                            ...
                            <li>Once your earnings reach a 30% return on your initial capital, you can withdraw a portion of your profits. For a ₦18,000 principal, this means you can withdraw ₦5,400, leaving you with at least your initial ₦18,000 balance to continue playing.</li>
                            ...
                            <li><strong>Opening Balance:</strong> The account balance at the start of a new betting period. For example, if your closing balance was ₦19,500 in the previous game and you stake ₦100 in the current game, your opening balance will be ₦9,400 in the current game.</li>
                                    ...
                                    <div key={index} className="contents">
                                        {week.map((day, idx) => (
                                            <button
                                                ...
                                                onClick={() => {
                                                    if (day !== '') {
                                                        fetchPoint5Data(day.toString(), selectedMonth, selectedYear);
                                                    }
                                                }}
                                                disabled={day === ''}
                                            >
